...
namespace Chaso.Push.Client
{
    public class ChasoPushClient : IChasoPushClient
    {
        ...
        public ChasoPushClient(string origin,string url)
        {
        ...
        private void Add(string channelName, string eventName, KeyValuePair<int, object> keyValuePair)
        {
            if (!_itemsToPush.Any(l => l.Key.Channel == channelName && l.Key.EventName == eventName))
                _itemsToPush.Add(new Models.Notify(Origin, channelName, eventName), new List<KeyValuePair<int, object>>());
        ...
        {
            var result = PushNotifyIntance.PushOperations.NotifyWithHttpMessagesAsync(new Models.Notify(Origin, channel, eventName, data)).Result;
